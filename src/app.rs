...
impl Hooks for App {
    ...
    async fn after_context(ctx: AppContext) -> Result<AppContext> {
        ...
        // SQLite has foreign keys disabled by default; PostgreSQL has them enabled by default
        if ctx.db.get_database_backend() == DatabaseBackend::Sqlite {
            ctx.db.execute_unprepared("PRAGMA foreign_keys = ON;").await?;
        }
        Ok(ctx)
    }
    ...
    fn routes(_ctx: &AppContext) -> AppRoutes {
        AppRoutes::with_default_routes() // controller routes below
            ...
            .add_route(controllers::statements::routes())
