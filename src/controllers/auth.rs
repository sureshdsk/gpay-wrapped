...
async fn register(
    ...
    // Auto-verify user in development mode only
    let is_dev = std::env::var("LOCO_ENV").unwrap_or_else(|_| "development".to_string()) == "development";

    if is_dev {
        user.into_active_model()
            .verified(&ctx.db)
            .await?;
        tracing::info!(email = &params.email, "user auto-verified in development");
    } else {
